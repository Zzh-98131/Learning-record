...
项目中有时会接入第三方服务，调用他们的接口。域名与当前的肯定是不一样的，记录一下配置方法和处理的跨域问题。

首先，配置`vue.config.js`，在proxy中配置**跨域域名**及**替代路径**等。
...
module.exports = {
	...
	devServer: {
    	...
    	proxy: {
    		"/api": { // 原有的api地址
        		target: process.env.VUE_APP_BASE_URL, // 这里使用了process，通过根目录的.env.xxx文件可以进行一些全局配置，例如VUE_APP_BASE_URL="http://xxxx.com"
            ...
            // 下面配置第三方域
            "/aixebao": {
  				target: "https://add1.pepcx.com/Ctl4ThirdParty",
  				// 在本地会创建一个虚拟服务端，然后发送请求的数据，并同时接收请求的数据，这样服务端和服务端进行数据的交互就不会有跨域问题
  				changeOrigin: true,
 				ws: true,
  				pathRewrite: {
    				// 替换target中的请求地址，也就是说以后你在请求https://add1.pepcx.com/Ctl4ThirdParty/xxx这个地址的时候直接写成/aixebao/xxx即可
    				"^/aixebao": "",
  				}
			}
    	}
...
配置好跨域之后，一般来说项目会封装`axios`请求，找到封装文件，这里要注意**可能需要对新的第三方域名进行请求拦截时的处理，在这里踩坑好久！**
...
const baseUrl = process.env.VUE_APP_BASE_URL; // 原本系统中配置的域

// 这部分并不是用的axios.create，不知道请求域名无法改变的原因是不是因为这里
...
#### width: fit-content，多个盒子并排排列，超出部分在可视区域内滑动

今天碰到一个需求，要做一个横向滑动的展示模块，发现了width一个好用的属性`fit-content`，记录一下。

基本思路：可视区域中放一个盒子`list`，`list`盒子的宽度由其中的子元素数量决定，子元素越多，盒子宽度越大。超出可视区域的部分滑动。难点在于盒子的横向排列。

有两种方法实现横向排列：一是`float`布局，但是`float`布局太影响其他页面元素了，而且浮动之后可视区域高度消失，不好做。二是自然而然会想到的`flex`布局。

`list`盒子使用`flex`布局后，其中的元素**可以并排展示**了，但是又出现了一个问题：子元素宽度之和小于`list`盒子宽度时，子元素宽度可以改变，但是当子元素宽度之和大于`list`盒子宽度时，继续增加子元素宽度，无效。

无效的原因是父盒子宽度不足，因此要增加`list`盒子的宽度，直接写死肯定不行，因为不知道子元素的数量，使用`width: auto;`也无效，最后，我发现使用`width: fit-content;`之后，`list`盒子的宽度就可以自适应了，是不是很神奇。
...
#### 多个div或li盒子并排时，消除重叠的边框

使用`margin`的负值，重点是上和下、左和右中，两个值一个为`0`一个为`-1`即可。
...
```js
filters: {
	filterWordOverFive: value => { // 超过5个字符的文本以省略号展示
...
```js
{{ '广州市天河中学高中部' | filterWordOverFive }} // 广州市天河…
{{ '广州市' | filterWordOverFive }} // 广州市
```
...
最近在做`vueapp`的`IOS`适配，绕了比较多弯路，记录一下。

背景：前端框架是`vue h5`，使用`cordova`将一套代码应用在微信公众号`web`端、安卓`app`和`IOSapp`上。

问题：安卓、微信`web`都没有出现屏幕适配的问题，唯独`IOS`。屏幕总是不适配，要么就是适配了之后呼出输入法，又回到原样。

第一次问题：画面占满屏幕，但是整体画面上移，底部留出空白，导致顶部按钮空间被压缩，不好操作。

解决方案：在样式中添加`IOS`的安全区适配代码。
...
第二次问题：添加代码后，刚进入`app`时画面不会上移了，但是当用户输入法弹出来时，适配又失效了。

解决方案：经过一番查找，发现`index.html`代码中有这么一行内容：

```html
 <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, viewport-fit=cover" /> 
```

当时也不明白这行代码具体起什么作用，但是注释掉之后，适配成功了，且不受输入法影响。

第三次问题：新的问题又出现了，`web`端页面现在可以放大缩小了，安卓`app`没事，`IOSapp`也可以放大缩小，用户体验很不好。。而且`IOSapp`部分地方的输入法呼出时，画面会自动放大。。

解决方案：百度一下，发现还是那行代码的问题，

```html
 <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, viewport-fit=cover" /> 
```

这行代码是用来控制用户缩放页面的程度的，其中`initial-scale`表示初始大小，`maximum-scale`表示可放大的最大程度，`minimum-scale`表示可缩小的最小程度，`viewport-fit`表示页面占满屏幕（估计`IOS`就是因为这个属性才变成沉浸式页面的）。解决也很简单，将最后一个属性`viewport-fit=cover`删除，问题就完美解决了。。。

### img图片等比缩小

图片缩小时可能会因为比例问题导致图片长宽比变形，对`img`使用**max-width**可以使图片等比缩放。
