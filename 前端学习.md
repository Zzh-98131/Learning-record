...
[process.env资料](https://yehaocheng520.blog.csdn.net/article/details/118382264?spm=1001.2101.3001.6661.1&utm_medium=distribute.pc_relevant_t0.none-task-blog-2%7Edefault%7ECTRLIST%7ERate-1-118382264-blog-121920480.pc_relevant_antiscanv2&depth_1-utm_source=distribute.pc_relevant_t0.none-task-blog-2%7Edefault%7ECTRLIST%7ERate-1-118382264-blog-121920480.pc_relevant_antiscanv2&utm_relevant_index=1)
...
只需要在项目启动命令后面修改需要的环境就行了，例如`npm run dev`，把`--mode dev`改为`--mode prod`就变成了正式环境
...
可以看到：在之前声明的WebFont路径中，原本大小10M的文件已经被替换为压缩后100多KB的文件，而压缩文件同级会生成一个.font-spider文件夹，里面存放的是原本大小的字体文件**备份**。另外，woff、eot等格式的字体文件也被同步生成在目录中。运行完font-spider命令后，可以**将声明代码注释掉**，避免上载之后路径报错。
...
export default {
    ...
    mounted() {
        this.deltaX = this.$refs.signHandle.getBoundingClientRect().x
        this.deltaY = this.$refs.signHandle.getBoundingClientRect().y
        this.draw();
    },
    methods: {
        /**
        ...
        drawStart(e) {
            ...
            this.ctx.beginPath();
            this.ctx.moveTo(
                e.changedTouches[0].pageX - this.deltaX,
                e.changedTouches[0].pageY - this.deltaY
            );
        },
        /**
         * 绘制过程
         ...
         */
        saveSign() {
            this.imagesBase64 = this.el.toDataURL();
            console.log("imagesBase64", this.imagesBase64);
        },
    },
};
</script>
...
async getList() {
	for (let i in 3) {
		await this.getParamList().then(res => {
            console.log(res, i)
        })
    }
}
...
接口编写，注意使用`trycatch`格式

```js
// 接口
async getParamList() { // 方法名为了方便定义为与接口同名
	let data = {}
    try {
		return await getParamList(data)
    } catch (e) {
        console.log(e)
        return []
    }
...
可以看到结果**按顺序输出**，证明异步等待有效，这种方法适用于**组装固定有顺序的数组**
...
如果只是简单地设置`position: absolute; bottom: 0;`来控制页脚置底，当页面高度过小，页面主体内容超出页面高度时，会出现页脚出现在画面底部，与页面主体内容重合的问题。

使用**flex布局**可以解决这个问题，让页脚在页面高度过小时可以滚动画面，在页面高度过大时保持页脚在页面底部。
...
https://www.jianshu.com/p/ba0058b77e9e
...
```css
.container {}
.cu_toolbar {display: none;}
.container:hover .cu_toolbar {display: block;}
```

#### 多个元素选中最后一行

**注意：元素顺序是从右下角到左上角，也就是顺序方向：下往上、右往左。**

**下方的类名一定要写完整，不能用&之类的符号代替！**
...
- `align-content`：定义多根轴线的对齐方式。如果项目只有一根轴线，该属性不起作用。
  **注意**：该属性需要配合`flex-direction: row; flex-wrap: wrap;`一起使用。
...
强推好文：https://juejin.cn/post/6844903512845860872
...
明确了第一点，就会明白JavaScript的异步事件都是模拟出来的，实际上依旧是单线程的执行核心。event loop就是事件执行的机制。事件循环的顺序，决定js代码的执行顺序。进入整体代码(**宏任务**)后，开始第一次循环。接着**执行所有的微任务**。然后再次从宏任务开始，找到**其中一个任务队列执行完毕**，再**执行所有的微任务**。

简单来说，JavaScript将任务分为：

- macro-task(宏任务)：包括整体代码script，**setTimeout**，**setInterval**
- micro-task(微任务)：Promise，process.nextTick

其中，宏任务又分**同步任务**和**异步任务**，比如setTimeout，setInterval就是异步任务。js先执行宏任务，再执行所有的微任务，执行宏任务时，先执行同步，再执行异步。
...
今天发现一个特别好玩的概念，可以用来提高系统性能，记录一下。

首先是**业务场景：**场景非常简单和常见，就是列表，以及列表上绑定的点击事件。以前在做列表点击事件时，都是将点击事件绑定在列表子项上，那么当列表有1000项，就绑定了1000个点击事件，比较消耗性能。那么可以利用js的事件冒泡机制，将子元素的事件绑定到父元素上，当点击子元素时，父元素可以捕捉到点击事件。

```vue
<div class="record" @click="goTasks"><!-- 点击事件绑定到这里 -->
    <div class="record-box" v-for="(task, i) in list" :key="i" :data-taskItem="JSON.stringify(task)">
        <svg-icon class="icon" icon-class="icon-task"></svg-icon>
        <span>{{ task.name }}</span>
        <svg-icon class="go" icon-class="icon-go"></svg-icon>
    </div>
</div>
```

这样一来，点击子元素时，父元素就能捕捉到当前点击事件了。只不过事件传值就不同于之前，之前点击事件绑定在子元素，可以直接传数组项数据到方法中，使用事件委托后，点击事件的传参就不是数据项了，而是事件对象。
...
// PointerEvent {isTrusted: true, pointerId: 11, width: 1, height: 1, pressure: 0, …}
```

那么怎么像以前一样获取到数据项呢？这里就要用到自定义属性了。首先，我们打印一下事件对象的`e.target.dataset`

```js
// DOMStringMap {v-36ac3b3a: ''}
```

可以看到里面只有一个莫名其妙的属性，如果我们在子元素上添加属性`:data-taskItem="JSON.stringify(task)"`，此时再次打印`e.target.dataset`

```js
// DOMStringMap {v-36ac3b3a: '', taskitem: '{"name":"您有一份学生成绩单未查看"}'}
```

对象中多出了我们刚刚自定义的`taskitem`属性，但是会**从驼峰体变成全小写**，还要注意的是，如果子元素上自定义的属性的传参是**基础数据类型**，那么可以直接传值不需要通过`JSON`转换：`:data-taskItem="JSON.stringify(task)"`。如果传参是**对象或者数组**，那么需要**加一层`JSON`转换**，使用时再转换回来。

```js
goTasks(e) {
    e = e || window.event // 兼容浏览器
    let { taskitem } = e.target.dataset
    console.log(JSON.parse(taskitem))
}
// {name: '您有一份学生成绩单未查看'}
```

数据层面是没问题了，但是实际使用时还是不方便，我们会发现，只有点击到事件所绑定的元素时，才能获取到传参，如果点击到事件元素下的子元素，就不能正常获取到值了。这样一来，除非将所有子元素都设定一个同样的自定义属性，否则点击时很有可能获取不到数据（事实上，就算所有子元素都设定了自定义属性，也不能保证百分百获取到值）。

我们可以通过给绑定元素添加一个遮罩层来解决这个问题：
...
首先`vuex`中使用模块化结构，假设`modules`文件夹中的`settting.js`文件。
...
然后是与`modules`文件夹同级的`getters.js`文件

```js
// getters.js
const getters = {
    getShow: state => state.settings.show
}
```

在页面中用`mapGetters`直接调用`vuex`中的属性，免去了重新定义

```js
import { mapGetters } from 'vuex';

computed: {
	...mapGetters(['getShow']) // 这里对应getters.js中的方法名
},
```
...
`Vue.directive`是我们除了内置的指令（如`v-model`和`v-show`）之外的自定义指令。自定义指令是对普通DOM元素进行的底层操作，它是一种有效的的补充和扩展，不仅可以用于定义任何的dom操作，并且是可以复用的
...
![](https://img-blog.csdnimg.cn/20201216160116314.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2xhbGFfbGlrZXN1bg==,size_16,color_FFFFFF,t_70)
...
1. vue.directive 有两个参数：id（string）和[definition
2. vue.directive有5种钩子函数，分别是：bind, inserted ,update, componentUpdated, unbind
   1. bind：只调用一次，指令第一次绑定到元素时调用，在这里可以进行一次性的初始化设置
   2. inserted：被绑定元素插入父节点时调用（仅保证父节点存在，但不一定被插入文档中）
...
使用this.$refs.itemName.**getBoundingClientRect()**，即可打印出元素位置信息对象
...
```js
this.$confirm(`确定创建${this.school}智能课表吗？`, "创建课表", {
...
return a[protoTypeName] - b[protoTypeName]
  }
}
...
[用法](https://www.jianshu.com/p/0272c0fe9392)
...
![\color{red}{注意：只有当组件在 <keep-alive> 内被切换，才会有activated 和 deactivated 这两个钩子函数}](https://math.jianshu.com/math?formula=%5Ccolor%7Bred%7D%7B%E6%B3%A8%E6%84%8F%EF%BC%9A%E5%8F%AA%E6%9C%89%E5%BD%93%E7%BB%84%E4%BB%B6%E5%9C%A8%20%3Ckeep-alive%3E%20%E5%86%85%E8%A2%AB%E5%88%87%E6%8D%A2%EF%BC%8C%E6%89%8D%E4%BC%9A%E6%9C%89activated%20%E5%92%8C%20deactivated%20%E8%BF%99%E4%B8%A4%E4%B8%AA%E9%92%A9%E5%AD%90%E5%87%BD%E6%95%B0%7D)
...
$.ajax({
  method: 'POST',
  async: false,
  url: Constant.getSoaPath("order") + 'getOrderList',
  contentType: "application/json; charset=utf-8", // 加上，才是请求荷载
  dataType: 'json',
  data: JSON.stringify(data), // 转字符串
  success: function (data) {
    console.log("getOrderList succ");
    reqResult = data;
  },
  error: function (err) {
...
在浏览器运行时，` -webkit-box-orient: vertical;`属性会消失，实际上是vue新脚手架版本不兼容的问题，需要在`.browserslistrc`文件中配置：
...
### 项目启动报错Cannot read property 'upgrade' of undefined

https://blog.csdn.net/weixin_44132285/article/details/121920480?spm=1001.2101.3001.6650.1&utm_medium=distribute.pc_relevant.none-task-blog-2%7Edefault%7ECTRLIST%7ERate-1-121920480-blog-103634494.pc_relevant_antiscanv2&depth_1-utm_source=distribute.pc_relevant.none-task-blog-2%7Edefault%7ECTRLIST%7ERate-1-121920480-blog-103634494.pc_relevant_antiscanv2&utm_relevant_index=2

项目部署到环境上去之后，突然发现本地环境请求接口报错了，一看，请求接口变成了`8080`本地，然后发现是`vue.config.js`文件没有配置跨域，刷刷配好之后，发现仍然`8080`，不对劲啊，遂重启项目，结果就报错`Cannot read property 'upgrade' of undefined`。

在`vue.config.js`文件设置启动项目时打印环境和接口地址，发现都是undefined，不对呀，`.env.dev`之类的文件都配置了（后来发现`.env.development`文件名是有效的，是我把它改成了`dev`。。）。然后开始百度，一开始说配置了`proxy`跨域后要设置`target`，这个没问题，`target`上也设置的是当前环境地址`process.env.VUE_APP_BASE_URL`，那么看来就是环境全局变量没有获取到的问题了。

定位好问题之后，查找了`vue`关于`process.env`全局变量的资料，发现了华点：`package.json`文件的启动命令中，没有配置启动的环境！

```js
"dev": "vue-cli-service serve --open",
```

这就导致项目启动时不知道自己在哪里，这简单，加句命令就好了

```js
"dev": "vue-cli-service serve --open --mode dev", // 添加--mode dev命令，其中dev是根据配置文件.env.dev后面的dev来明确
```

### 点击返回按钮时，页面跳转不出去，然后刷新本页面

检查一下是否写了`beforeRouteLeave`。。。

### IOS的Web端呼出输入法后底部留有大片空白
...
首先安装`Paste and Indent`插件，然后复制以下代码：
...
先按住`crtl`，然后分别按`k`和`s`，唤出键盘快捷方式的设置，然后选择打开设置的`json`文件，将上面代码添加到末尾即可。

### 设置vue模板

使用`Ctrl + Shift + P`呼出控制台，然后输入`snippets`

在出现的框中输入 `vue` 之后按回车键, 出现 `vue.json` 文件
...
vuex中的store分模块管理，需要在store的index.js中引入各个模块，为了解决不同模块**命名冲突**的问题，将不同模块的`namespaced:true`，之后在不同页面中引入getter、actions、mutations时，**需要加上所属的模块名**

**声明分模块的store时加上namespaced:true**
...
https://www.jianshu.com/p/09514407af97

经常有将接口返回的数据作为基础数据保存到vuex的场景，若这部分基础数据是同页面其他接口需要的参数，当基础数据保存到本地不够及时，那么其他使用到基础数据的接口就会报**null**错误。这是因为保存到本地需要时间，即使对接口做了异步等待处理，也会存在保存数据过程中的时间差。

解决方法：保存数据到vuex时，不要用同步的`mutations`操作，而是用支持异步的`actions`操作。这里就要提到二者的差别，`mutations`直接对vuex里的state数据做修改，而`actions`触发后，还要再经过`mutations`才能对数据做修改，即`this.$store.dispatch() = actions -> mutations -> state`，当然，第一步`actions`可以省略，那就变成了`this.$store.commit()`了。

使用方法：`actions`与`mutations`不同，`mutations`中的方法携带形参是`state`和传入的自定义数据。`actions`的形参是`context`和`payload`，即当前的`store`和自定义传参。下面演示一下异步保存
...
var saveData = async function() {
    ...
    console.log(1)
	await this.$store.dispatch('setData', obj) // 携带数据到异步操作中
    console.log(2)
}
saveData()
...
// store
mutations: {
    setData: (state, data) =>{ // 第一个参数是状态树state，第二个数据是自带参数
        state.data = data;
      	localStorage.setItem("data", JSON.stringify(data));
    }
},
actions: {
	setData: (context, data) => { // 第一个参数是复制的store，第二个数据是自带参数
    // setData: ({ commit, state }, data) => { // 也可以使用更加简洁的参数解构写法，直接commit('setData')
        // 既然支持异步，那么就可以返回Promise对象，这里人为设置一个异步时长，5秒
...
众所周知，`vue`页面刷新后，`vuex`里的内容都会变成`null`。这有可能导致`getters`不更新，比如那些通过`getters`来获取的值，就不能通过赋值`null`来置空，因为`getters`相当于计算属性，`state`有变化时才会触发。

**解决方法：页面刷新前，将store的内容存入会话缓存，刷新完毕之后再从会话缓存中获取值**

将以下代码放在App.vue的created里
...
**另：**还有一种情况，当app端关闭了页面时（例如微信web端），再打开页面，此时vuex中数据为null，此时退出登录将信息置为null会导致getters不刷新。

**解决方法：置空不要写null，写其他可以判空的数据，比如''。**
